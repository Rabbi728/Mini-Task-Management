...
import { ArrowLeft, Mail, Save, CheckCircle2, AlertCircle, Loader2 } from "lucide-react";
...
const userSchema = yup.object().shape({
  ...
  password: yup.string().min(6, "Password must be at least 6 characters").nullable().transform((v) => (v === "" ? null : v)),
});
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setApiError("Failed to load user data. It might not exist.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof yup.ValidationError) {
        ...
        setApiError(err.response?.data?.message || "Failed to update user. Please try again.");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1 font-medium">Update profile details for {formData.name}.</p>
      </div>
      ...
      {success && (
        <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-2xl flex items-center gap-4 text-emerald-800 shadow-sm animate-in zoom-in-95 duration-300">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm opacity-90">The user information has been synchronized.</p>
          </div>
        </div>
      )}
