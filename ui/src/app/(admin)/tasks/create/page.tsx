...
import { ArrowLeft, Save, CheckCircle2, AlertCircle, Layout, AlignLeft, User, ListChecks } from "lucide-react";
...
export default function CreateTaskPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/tasks`, validatedData, { withCredentials: true });
      setSuccess(true);
      setFormData({ title: "", description: "", status: "PENDING", assigned_user: "" });
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-700">
      ...
      <div>
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Initialize New Task</h1>
        <p className="text-slate-500 mt-1 font-medium">Define a new objective and assign it to a team member.</p>
      </div>
      ...
      {success && (
        <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-2xl flex items-center gap-4 text-emerald-800 shadow-sm animate-in zoom-in-95 duration-300">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm opacity-90">The new task has been added to the system.</p>
          </div>
          <button 
